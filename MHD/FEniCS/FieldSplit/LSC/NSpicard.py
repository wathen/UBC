...
for x in xrange(1,4):
    ...
    for xx in xrange(1,m):
        ...
        if Solver == "LSC":
            ...
            fp = inner(grad(qQ), grad(pQ))*dx+inner((u_k[0]*grad(pQ)[0]+u_k[1]*grad(pQ)[1]),qQ)*dx + (1/2)*div(u_k)*inner(pQ,qQ)*dx - (1/2)*(u_k[0]*n[0]+u_k[1]*n[1])*inner(pQ,qQ)*ds
            # print "hi"
            L = CP.Assemble(L)
...
LatexTitles = ["DoFu","Dofp","AvIt"]
LatexValues = np.concatenate((Vdim,Qdim,AvIt), axis=1)
