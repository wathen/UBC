...
for xx in xrange(1,m):
    ...
    if Solver == "LSC":
        ...
        fp = MuU*inner(grad(qQ), grad(pQ))*dx+inner((u_k[0]*grad(pQ)[0]+u_k[1]*grad(pQ)[1]),qQ)*dx + (1/2)*div(u_k)*inner(pQ,qQ)*dx - (1/2)*(u_k[0]*n[0]+u_k[1]*n[1])*inner(pQ,qQ)*ds
        # print "hi"
        L = CP.Assemble(L)
    ...
    while eps > tol and iter < maxiter:
        ...
        ksp.setOperators(A)
        OptDB = PETSc.Options()
        OptDB['pc_factor_shift_amount'] = .00001
    ...
    const = - assemble(pInterp*dx)/assemble(ones*dx)
    pe.vector()[:] = pInterp.vector()[:]+const
