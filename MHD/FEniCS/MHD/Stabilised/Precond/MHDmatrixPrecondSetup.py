...
def FluidLinearSetup(Pressure,mu):
    ...
    L = assemble(mu*(inner(grad(q), grad(p))*dx \
                            - inner(avg(grad(q)), outer(p('+'),N('+'))+outer(p('-'),N('-')))*dS \
                            - inner(outer(q('+'),N('+'))+outer(q('-'),N('-')), avg(grad(p)))*dS \
                            + alpha/h_avg*inner(outer(q('+'),N('+'))+outer(q('-'),N('-')),outer(p('+'),N('+'))+outer(p('-'),N('-')))*dS \
                            - inner(outer(q,N), grad(p))*ds \
                            - inner(grad(q), outer(p,N))*ds \
                            + gamma/h*inner(q,p)*ds))
...
def FluidNonLinearSetup(Pressure,mu, u_k):
    ...
    tic()
    Fp = assemble(mu*(inner(grad(q), grad(p))*dx \
                            - inner(avg(grad(q)), outer(p('+'),N('+'))+outer(p('-'),N('-')))*dS \
                            - inner(outer(q('+'),N('+'))+outer(q('-'),N('-')), avg(grad(p)))*dS \
                            + alpha/h_avg*inner(outer(q('+'),N('+'))+outer(q('-'),N('-')),outer(p('+'),N('+'))+outer(p('-'),N('-')))*dS \
                            - inner(outer(q,N), grad(p))*ds \
                            - inner(grad(q), outer(p,N))*ds \
                            + gamma/h*inner(q,p)*ds) \
                            + inner(inner(grad(p),u_k),q)*dx- (1/2)*inner(u_k,N)*inner(q,p)*ds \
                            -(1/2)*(inner(u_k('+'),N('+'))+inner(u_k('-'),N('-')))*avg(inner(q,p))*ds \
                            -dot(avg(q),dot(outer(p('+'),N('+'))+outer(p('-'),N('-')),avg(u_k)))*dS)
