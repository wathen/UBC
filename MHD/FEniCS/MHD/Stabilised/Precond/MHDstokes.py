...
for xx in xrange(1,m):
    ...
    F_NS = -MU*Laplacian+gradPres-kappa*NS_Couple
    if kappa == 0:
    ...
    HiptmairMatrices = PrecondSetup.MagneticSetup(Magnetic, Lagrange, b0, r0, 1e-4)
    ...
    ns,maxwell,CoupleTerm,Lmaxwell,Lns = forms.MHD2D(mesh, W,F_M,F_NS, u_k,b_k,params,IterType,"DG", Stokes = "Yes")
    RHSform = forms.PicardRHS(mesh, W, u_k, p_k, b_k, r_k, params,"DG",Stokes = "Yes")
    ...
    while eps > tol  and iter < maxiter:
        ...
        if IterType == "CD":
            ...
            ShiftedMass = assemble(kappa*MU*inner(curl(b_t),curl(c_t))*dx + inner(b_t,c_t)*dx+kappa*kappa/MU*inner(mat*b_t,c_t)*dx)
        ...
        ksp.max_it = 1000
        FFSS = [Velocity,Pressure,Magnetic,Lagrange]
        pc.setPythonContext(MHDstabPrecond.SaddleFromTest(FFSS,kspF, KSPlinearfluids[0], KSPlinearfluids[1],Fp, HiptmairMatrices[3], HiptmairMatrices[4], HiptmairMatrices[2], HiptmairMatrices[0], HiptmairMatrices[1], HiptmairMatrices[6],1e-6,FF))
        # OptDB = PETSc.Options()
        ...
        u1, p1, b1, r1, eps= Iter.PicardToleranceDecouple(u,x,FFSS,dim,"2",iter)
