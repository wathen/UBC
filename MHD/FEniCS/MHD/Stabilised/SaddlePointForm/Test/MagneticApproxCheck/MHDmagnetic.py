...
for xx in xrange(1,m):
    ...
    bcb3 = DirichletBC(Magnetic,Expression(("1.0","1.0")), boundary)
    ...
    while eps > tol  and iter < maxiter:
        ...
        if IterType == "CD":
            ...
            ShiftedMass = assemble(kappa*Mu_m*inner(curl(b_t),curl(c_t))*dx+inner(b_t,c_t)*dx+kappa*kappa/MU*inner(mat*b_t,c_t)*dx)
            ShiftedMass = assemble(kappa*kappa/MU*inner(mat*b_t,c_t)*dx)
            bcb2.apply(ShiftedMass)
        ...
        pc.setPythonContext(MHDpreconditioner.InnerOuterWITHOUT2inverse(FFSS,kspF, KSPlinearfluids[0], KSPlinearfluids[1],Fp, HiptmairMatrices[3], HiptmairMatrices[4], HiptmairMatrices[2], HiptmairMatrices[0], HiptmairMatrices[1], HiptmairMatrices[6],1e-6,FF))
        # OptDB = PETSc.Options()
        ...
        Schur = pc.getPythonContext().SchurComp()
        ...
        BC = sp.diags(b/scale,0,(Magnetic.dim(),Magnetic.dim()))
        schur = CP.PETSc2Scipy(Schur)
        schur += BC
        Mass = CP.PETSc2Scipy(FF)
        
        bschur = Function(Magnetic)
        bmass = Function(Magnetic)
        bschur.vector()[:] = schur*bb
        bmass.vector()[:] = Mass*bb

        e = bschur-bmass
        EE = np.sqrt(assemble(inner(e,e)*dx))
        MO.PrintStr(str(np.linalg.norm(schur*bb-Mass*bb)),100,'=','\n','\n')
        MO.PrintStr(str(EE),100,'=','\n','\n')
