...
def FluidNonLinearSetup(Pressure,mu, u_k):
    ...
    if Pressure.__str__().find("CG") == -1:
        ...
        if mesh.topology().dim() == 2:
            Fp = assemble(mu*inner(grad(q), grad(p))*dx(mesh)+inner((u_k[0]*grad(p)[0]+u_k[1]*grad(p)[1]),q)*dx(mesh))
            print u_k.shape(),(p).shape()
            inner(u_k,N)
        else:
            Fp = assemble(mu*inner(grad(q), grad(p))*dx(mesh)+inner((u_k[0]*grad(p)[0]+u_k[1]*grad(p)[1]+u_k[2]*grad(p)[2]),q)*dx(mesh))
