...
def foo():
    ...
    MUsave = np.zeros((1,3*(mm)))
    MUits = np.zeros((m-1,3*(mm)))
    nn = 2
    ...
    for xx in xrange(1,m):
        ...
        for yy in xrange(1,mm+1):
            ...
            for jj in range(3):
                ...
                Velocity = VectorFunctionSpace(mesh, "CG", order) + VectorFunctionSpace(mesh, "B", 3)
                ...
                ns,maxwell,CoupleTerm,Lmaxwell,Lns = forms.MHD2D(mesh,FSpaces,F_M,F_NS, u_k,b_k,params,IterType,"CG",Saddle,Stokes)
                RHSform = forms.PicardRHS(mesh, FSpaces, u_k, p_k, b_k, r_k, params,"CG",Saddle,Stokes)

                bcu = DirichletBC(FSpaces[0],Expression(("0.0","0.0")), boundary)
                bcb = DirichletBC(FSpaces[2],Expression(("0.0","0.0")), boundary)
                ...
                while eps > tol  and iter < maxiter:
                    ...
                    if  Split == "Yes":
                        if iter == 1:
                            Fnlin,b, bc = MHDsetup.Assemble(W,ns,maxwell,CoupleTerm,Lns,Lmaxwell,RHSform,bcs+BC, "NonLinear",IterType)
                            BC[0] = bc
                            Alin = MHDsetup.Assemble(W,ns,maxwell,CoupleTerm,Lns,Lmaxwell,RHSform,bcs+BC, "Linear",IterType)
                            A = Fnlin+Alin
                            A,b = MHDsetup.SystemAssemble(FSpaces,A,b,SetupType,IterType)
                            ...
                            Fnline,b,bc = MHDsetup.Assemble(W,ns,maxwell,CoupleTerm,Lns,Lmaxwell,RHSform,bcs+BC, "NonLinear",IterType)
                            A = Fnlin+Alin
                            A,b = MHDsetup.SystemAssemble(FSpaces,A,b,SetupType,IterType)
                    else:
                    ...
                    stime = time.time()
                    u, mits,nsits = S.solve(A,b,u,params,W,'Direct',IterType,OuterTol,InnerTol,HiptmairMatrices,Hiptmairtol,KSPlinearfluids, Fp,0)
...
#    print MagneticTable
    ...
    pd.set_option('precision',3)
    LatexValues = np.concatenate((level,Wdim,MUits), axis=1)
