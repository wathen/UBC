...
for xx in xrange(1,m):
    ...
    bcb = DirichletBC(MagneticF, Expression(("0.0", "0.0"), degree=4), boundary)
    ...
    scipy.io.savemat("Matrix/bcu_"+str(int(level[xx-1][0]))+".mat", mdict={"bcu": bcu})
    scipy.io.savemat("Matrix/bcp_"+str(int(level[xx-1][0]))+".mat", mdict={"bcp": bcp})
    scipy.io.savemat("Matrix/bcb_"+str(int(level[xx-1][0]))+".mat", mdict={"bcb": bcb})
    scipy.io.savemat("Matrix/bcr_"+str(int(level[xx-1][0]))+".mat", mdict={"bcr": bcr})
    scipy.io.savemat("Matrix/dim_"+str(int(level[xx-1][0]))+".mat", mdict={"bcr": np.array([VelocityF.dim(), PressureF.dim(), MagneticF.dim(), LagrangeF.dim()])})
