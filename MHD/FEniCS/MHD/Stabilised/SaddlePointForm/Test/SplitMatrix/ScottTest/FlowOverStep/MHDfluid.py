...
for xx in xrange(1,m):
    ...
    y0 = 2.
    z0 = 1.
    mesh, boundaries, domains = FlowOverStep.Domain(nn)
    ...
    trunc = 4
    u0, b0, p0, r0 = FlowOverStep.ExactSol(mesh, params)
    ...
    u_k, p_k = FlowOverStep.Stokes(Velocity, Pressure, F_S, u0, p0, params, boundaries, domains)
    b_k, r_k = FlowOverStep.Maxwell(Magnetic, Lagrange, F_M, b, r0, params, boundaries,HiptmairMatrices, Hiptmairtol)
    ...
    CoupleT = params[0]*(v[0]*b_k[1]-v[1]*b_k[0])*curl(b)*dx('everywhere')
    Couple = -params[0]*(u[0]*b_k[1]-u[1]*b_k[0])*curl(c)*dx('everywhere')

    a = m11 + m12 + m21 + a11 + a21 + a12 + Couple + CoupleT

    Lns  = inner(v, F_S)*dx('everywhere') + inner(p0*n,v)*ds(2)
    ...
    CoupleT = params[0]*(v[0]*b_k[1]-v[1]*b_k[0])*curl(b_k)*dx('everywhere')
    Couple = -params[0]*(u_k[0]*b_k[1]-u_k[1]*b_k[0])*curl(c)*dx('everywhere')
