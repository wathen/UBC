...
def foo():
    ...
    for xx in xrange(1,m):
        ...
        while eps > tol  and iter < maxiter:
            ...
            aVec, L_M, L_NS, Bt, CoupleT = forms.MHDmatvec(mesh, W, Laplacian, Laplacian,u_k,b_k,u,b,p,r, params,"Full","CG", SaddlePoint = "No")
            bcu = DirichletBC(Velocity,u0, boundary)
            PrecondTmult = {'Bt': Bt, 'Ct':CoupleT, 'BC': bcu}
            FS = {'velocity': Velocity, 'pressure': Pressure, 'magnetic': Magnetic, 'multiplier': Lagrange}
            ...
            P.setUp()
            stime = time.time()
            u, mits,nsits = S.solve(A, P,b,u,params,W,'Directsss',IterType,OuterTol,InnerTol,HiptmairMatrices,Hiptmairtol,KSPlinearfluids, Fp,kspF)
