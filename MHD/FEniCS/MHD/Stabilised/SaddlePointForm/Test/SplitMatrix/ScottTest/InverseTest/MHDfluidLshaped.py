...
for xx in xrange(1,m):
    ...
    # Qs = Qs.sparray()
    # Qs.eliminate_zeros()
    # Qs = MO.StoreMatrix(Qs, 'Qs'+str(int(level[xx-1][0]))+'_Lshape')
    # X = X.sparray()
    # X.eliminate_zeros()
    # X = MO.StoreMatrix(X, 'X'+str(int(level[xx-1][0]))+'_Lshape')

    # Mp = Mp.sparray()
    # Mp.eliminate_zeros()
    # Mp = MO.StoreMatrix(Mp, 'Mp'+str(int(level[xx-1][0]))+'_Lshape')
    # Fp = Fp.sparray()
    # Fp.eliminate_zeros()
    # Fp = MO.StoreMatrix(Fp, 'Fp'+str(int(level[xx-1][0]))+'_Lshape')
    ...
    # Q = Q.sparray()
    # Q.eliminate_zeros()
    # Q = MO.StoreMatrix(Q, 'Q'+str(int(level[xx-1][0]))+'_Lshape')

    # A = A.sparray()
    # A.eliminate_zeros()
    # A = MO.StoreMatrix(A, 'A'+str(int(level[xx-1][0]))+'_Lshape')
    # O = O.sparray()
    # O.eliminate_zeros()
    # O = MO.StoreMatrix(O, 'O'+str(int(level[xx-1][0]))+'_Lshape')

    # Xs = Xs.sparray()
    # Xs.eliminate_zeros()
    # Xs = MO.StoreMatrix(Xs, 'Xs'+str(int(level[xx-1][0]))+'_Lshape')
    ...
    # np.savetxt('vBoundary'+str(int(level[xx-1][0]))+'_Lshape'+".t",np.array(VelocityBoundary.keys()))
    # np.savetxt('bBoundary'+str(int(level[xx-1][0]))+'_Lshape'+".t",np.array(MagneticBoundary.keys()))
    # np.savetxt('rBoundary'+str(int(level[xx-1][0]))+'_Lshape'+".t",np.array(MultiplierBoundary.keys()))


np.savetxt('dimensions_Lshaped.t',Dimensions)
