...
import ExactSol
import Solver as S
import MHDmatrixPrecondSetup 
import NSprecondSetup
import MHDpreconditioner
import MHDschurCompApprox
import PrecondSetup
m = 6
...
for xx in xrange(1,m):
    ...
    HiptmairMatrices = MHDmatrixPrecondSetup.MagneticSetup(Magnetic, Lagrange, b0, r0, 1e-4)
    ...
    KSPlinearfluids, MatrixLinearFluids = MHDmatrixPrecondSetup.FluidLinearSetup(Pressure, MU)
    kspFp, Fp = MHDmatrixPrecondSetup.FluidNonLinearSetup(Pressure, MU, u_k)
    ...
    bcb2 = DirichletBC(Magnetic,Expression(("0.0","0.0")), boundary)
    ...
    b_is = PETSc.IS().createGeneral(range(Velocity.dim(),Velocity.dim()+Magnetic.dim()))
    ...
    while eps > tol  and iter < maxiter:
        ...
        kspFp, Fp = MHDmatrixPrecondSetup.FluidNonLinearSetup(Pressure, MU, u_k)
        SchurComp = PrecondSetup.TensorMass(b_k,params,Magnetic)
        ...
        pc.setPythonContext(MHDschurCompApprox.TensorMass(FFSS,kspF, KSPlinearfluids[0], KSPlinearfluids[1],Fp, HiptmairMatrices[3], HiptmairMatrices[4], HiptmairMatrices[2], HiptmairMatrices[0], HiptmairMatrices[1], HiptmairMatrices[6],1e-4,SchurComp))
        # OptDB = PETSc.Options()
...
if SchurComp == Velocity.dim():
    MO.PrintStr("Iteration table for tensor weighted velocity mass matrix",5,"-","\n","\n")
else:
    MO.PrintStr("Iteration table for tensor weighted magnetic mass matrix",5,"-","\n","\n")
