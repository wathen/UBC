...
from dolfin import *
import numpy
from scipy.sparse import coo_matrix, block_diag, hstack, tril, spdiags, csr_matrix, vstack
import ExactSol
...
for xx in xrange(1,m):
    ...
    if dim == 3:
        ...
        bcW = DirichletBC(W.sub(0),Expression(("2.0","1.0")), boundary)
    ...
    Grad = assemble(inner(grad(q),u)*dx)
    Laplacian = assemble(inner(grad(q),grad(p))*dx)
    # # plt.spy(C)
    # # plt.show()
    bc.apply(Curl)
    bc.apply(Mass)
    # # print Diaglagrange*Grad.sparray()*Diagmagnetic*C
    # print Diaglagrange*Laplacian.sparray()*Diaglagrange
    # bcu.apply(Laplacian)

    # plt.figure()
    # plt.subplot(1,5,1)
    # plt.title("B")
    # plt.spy(Diaglagrange*Grad.sparray()*Diagmagnetic)
    # plt.subplot(1,5,2)
    # plt.title("C")
    # plt.spy(C)
    # plt.subplot(1,5,3)
    # plt.title("B*C")
    # plt.spy(Diaglagrange*Grad.sparray()*Diagmagnetic*C)
    # plt.subplot(1,5,4)
    # plt.title("L")
    # plt.spy(Laplacian.sparray())
    # plt.subplot(1,5,5)
    # plt.title("B*C-L")
    # plt.spy(Diaglagrange*Grad.sparray()*Diagmagnetic*C-Laplacian.sparray())
    # print Laplacian.sparray().todense()
    print  "========================================"
    Curlgrad[xx-1]=(Curl.sparray()*C).max()
    print " Curl-gradient test: ", Curlgrad[xx-1]
    Massgrad[xx-1]=(Mass.sparray()*C-Diagmagnetic*Grad.sparray().transpose()*Diaglagrange).max()
    print " Mass-gradient test: ", Massgrad[xx-1]
    Laplgrad[xx-1]=(Diaglagrange*Grad.sparray()*Diagmagnetic*C-Diaglagrange*Laplacian.sparray()*Diaglagrange).max()
    ...
    Zero = Expression(("0","0"))
    uuu = DirichletBC(VLagrange, Expression(("0","0")), boundary)
    vvv = DirichletBC(Magnetic, Expression(("0","0")), boundary)
    Func = Expression(("sin(pi*x[0])*sin(pi*x[1])","sin(pi*x[0])*sin(pi*x[1])"))
    VV = interpolate(Func,VLagrange)
    Ev = interpolate(Func,Magnetic)
    P = hstack([Diagmagnetic*Px*Diaglagrange,Diagmagnetic*Py*Diaglagrange]).tocsr()
    print hstack([Diaglagrange,Diaglagrange]).shape
    print Diaglagrange.dot(VV.vector().array()[Lagrange.dim():]).shape
    BCVV = numpy.concatenate([BClagrange,BClagrange], axis=1)
    print BCVV.shape
    VV = VV.vector()
    uuu.apply(VV)
    # VV.vector().array()[BCVV] = 0
    # print VV.vector().array()
    # plt.show()
    EdgeDoF =  P*VV.array()
    # EdgeDoFY =  Py*VertexDoF
    EE = Function(Magnetic)
    EE.vector()[:] = Diagmagnetic*EdgeDoF
    VE = Function(VLagrange)
    VE.vector()[:] = VV.array()
    # file << VV

    # plot(EE,tite="Magnetic interpolation")
    # # plot(EEY,tite="Magnetic interpolation Y-direction")
    # plot(VE,tite="Nodal represetation")
    parameters["form_compiler"]["quadrature_degree"] = 5
    InterpError[xx-1] = sqrt(abs(assemble(inner(curl(EE-VE),curl(EE-VE))*dx)))
    print "==============================",    InterpError[xx-1]
    # print (Px).todense()
    # print (Pxx).todense()
    if dim == 2:
        tic()
    ...
    if xx == 1:
        ...
        InterpOrder[xx-1] =  numpy.abs(numpy.log2(InterpError[xx-2]/InterpError[xx-1]))
