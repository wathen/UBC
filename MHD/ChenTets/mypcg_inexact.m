...
function [x,flag,relres,iter,resvec] = mypcg(A,b,tol,maxit,M1,M2,x0,W,B,varargin)
...
%   Alternatively, use this parameterized matrix-vector product function:
...
%   $Revision: 1.18.4.3 $ $Date: 2004/06/25 18:52:27 $
...
if isequal(atype,'matrix')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   %disp('[MYPCG]: computing initial residual')
   r = b - (A * x + B' * (F \ (F' \ (B*x))));                  % Zero-th residual
   %disp('[MYPCG]: computing residual')
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   %disp('test!!')
   %test1=A * x + B' * (F \ (F' \ (B*x)));
   %test2=(A+B'*(W\B))*x;
   %norm(test1-test2),pause
else
...
for i = 1 : maxit
   if existM1
      if isequal(m1type,'matrix')
         %y = M1 \ r;
         toly=10^(-8+i);
         [y,flagy,relresy,itery,resvecy]=pcg(M1,r,toly,1000,F1',F1);
         flagy,itery
      else
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      ...
      if mod(i,100) == 0
        i
        %disp('[MYPCG]: computing mat-vec product')
      end
      q = (A * p + B' * (F \ (F' \ (B*p))));
